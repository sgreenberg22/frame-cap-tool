...
export default function ExportGIF({ videoRef, filterCss, shots }){
  ...
  async function makeFramesFromVideo(){
    ...
    for(let t = start; t <= end + 1e-6; t += step){
      ...
      if(applyFilters) ctx.filter = filterCss; else ctx.filter = 'none'
  ...
  function rgbaToRgb(imageData){
    ...
    const out = new Uint8Array((src.length/4)*3)
    for(let i=0,j=0;i<src.length;i+=4,j+=3){ out[j]=src[i]; out[j+1]=src[i+1]; out[j+2]=src[i+2] }
    return out
  }
  ...
  async function generate(){
    ...
    try{
      ...
      for(let i=0;i<frames.length;i++){
        ...
        const rgb = rgbaToRgb(frames[i])
        const pal = quantize(rgb, 256)
        const idx = applyPalette(rgb, pal)
        enc.writeFrame(idx, frames[i].width, frames[i].height, { palette: pal, delay, transparent: false, repeat: loop? 0 : -1 })
        await microtask()
      }
      enc.finish()
